...
export class InternDetailComponent implements OnInit {
 ...
 get role(){return this.auth.currentUserSnapshot?.role;}get canAdmin(){return this.role==='SUPER_ADMIN';}get canSupervise(){return this.role==='EMPLOYEE'||this.role==='TRAINER_TUTOR';}get canUpload(){return this.canAdmin||this.role==='INTERN';}get canEvaluate(){return this.canAdmin||this.canSupervise;}
 ...
 declarePhysical(requirement:InternDocumentRequirement){if(!this.intern)return;this.service.declarePhysicalDocument(this.intern.id,requirement.id).subscribe({next:()=>{this.notice('Remise physique enregistrée.');this.load(this.intern!.id);},error:e=>this.notice(e.error?.detail??'Déclaration impossible.')});}
 submissionLabel(requirement:InternDocumentRequirement){const method=requirement.latest_submission?.submission_method;return method==='ONLINE'?'Déposé en ligne':method==='PHYSICAL'?'Remis physiquement':'Manquant';}
 submissionIcon(requirement:InternDocumentRequirement){const method=requirement.latest_submission?.submission_method;return method==='ONLINE'?'cloud_done':method==='PHYSICAL'?'inventory_2':'error_outline';}
