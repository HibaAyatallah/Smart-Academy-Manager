...
@Component({ selector: 'app-enrollment-workflow', standalone: true, imports: [CommonModule, ReactiveFormsModule, MatButtonModule, MatCardModule, MatChipsModule, MatFormFieldModule, MatInputModule, MatPaginatorModule, MatSelectModule, MatSnackBarModule, PageHeaderComponent], templateUrl: './enrollment-workflow.component.html', styleUrl: './enrollment-workflow.component.scss' })
export class EnrollmentWorkflowComponent implements OnInit {
  ...
  enrollments: TrainingEnrollment[] = []; loading = true; error = ''; total = 0; pageIndex = 0; readonly pageSize = 20;
  ...
  load(page = 1): void { this.loading = true; this.error = ''; this.service.getEnrollments({...this.filters.getRawValue(), page}).pipe(finalize(() => this.loading = false)).subscribe({ next: data => { this.enrollments = data.results; this.total = data.count; this.pageIndex = page - 1; }, error: () => this.error = 'Impossible de charger les inscriptions.' }); }
