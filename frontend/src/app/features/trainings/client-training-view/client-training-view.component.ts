...
export class ClientTrainingViewComponent implements OnInit { private readonly service=inject(TrainingService); readonly labels=SESSION_STATUS_LABELS; trainings:ClientTraining[]=[]; loading=true; error=''; ngOnInit(){this.service.getAllClientTrainings().subscribe({next:trainings=>{this.trainings=trainings;this.loading=false;},error:()=>{this.error='Impossible de charger vos formations.';this.loading=false;}});} safe(url:string){try{const parsed=new URL(url);return ['http:','https:'].includes(parsed.protocol)?url:'';}catch{return '';}} }
