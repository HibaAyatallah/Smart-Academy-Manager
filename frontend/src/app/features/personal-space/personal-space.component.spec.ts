...
describe('PersonalSpaceComponent', () => {
  ...
  beforeEach(async()=>{
    auth=Object.assign(jasmine.createSpyObj<AuthService>('AuthService',['changePassword','updateContactDetails','logout','getDashboardUrlForRole']),{currentUser$:new BehaviorSubject<UserProfile|null>(user),currentUserSnapshot:user});
    auth.updateContactDetails.and.returnValue(of(user));auth.changePassword.and.returnValue(of(user));auth.getDashboardUrlForRole.and.returnValue('/dashboard/hr');
  ...
  it('navigates to the dashboard after a confirmed password change',()=>{spyOn(window,'confirm').and.returnValue(true);component.passwordForm.setValue({current_password:'Current123!',new_password:'NewStrong456!',confirmation:'NewStrong456!'});component.changePassword();expect(auth.changePassword).toHaveBeenCalled();expect(auth.getDashboardUrlForRole).toHaveBeenCalledWith('HR');});
});
